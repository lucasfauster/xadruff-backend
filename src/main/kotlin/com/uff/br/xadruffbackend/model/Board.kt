...
class Board {
    ...
    private fun calculateLegalMoves() {
        ...
        fun calculateBishopMoves(line: Int, col: Int){
            var upLeftDiagonalFree= true; var downLeftDiagonalFree = true; var upRithDiagonalFree = true; var downRightDiagonalFree = true
            for( i in (1..7)){
                if(hasAllie(line+i, col-i) || isOutOfIndex(line+i, col-i)) downLeftDiagonalFree = false
                if(isEmpty(line+i, col-i) && downLeftDiagonalFree) addNewMove(line, col, line+i, col-i)
                if(hasEnemy(line+i, col-i) && downLeftDiagonalFree) {
                    addNewMove(line, col, line + i, col-i, "C")
                    downLeftDiagonalFree = false
                }

                if(hasAllie(line-i, col-i) || isOutOfIndex(line-i, col-i)) upLeftDiagonalFree = false
                if(isEmpty(line-i, col-i) && upLeftDiagonalFree) addNewMove(line, col, line-i, col-i)
                if(hasEnemy(line-i, col-i) && upLeftDiagonalFree) {
                    addNewMove(line, col, line - i, col-i, "C")
                    upLeftDiagonalFree = false
                }

                if(hasAllie(line+i, col+i) || isOutOfIndex(line+i, col+i)) downRightDiagonalFree = false
                if(isEmpty(line+i, col+i) && downRightDiagonalFree) addNewMove(line, col, line+i, col+i)
                if(hasEnemy(line+i, col+i) && downRightDiagonalFree) {
                    addNewMove(line, col, line+i, col + i, "C")
                    downRightDiagonalFree = false
                }

                if(hasAllie(line-i, col+i) || isOutOfIndex(line-i, col+i)) upRithDiagonalFree = false
                if(isEmpty(line-i, col+i) && upRithDiagonalFree) addNewMove(line, col, line-i, col+i)
                if(hasEnemy(line-i, col+i) && upRithDiagonalFree) {
                    addNewMove(line, col, line-i, col + i, "C")
